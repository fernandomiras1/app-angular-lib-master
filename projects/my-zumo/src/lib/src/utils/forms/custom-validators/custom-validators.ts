...
import { all } from '../telephone/json.var';
...
export class CustomValidators {
  ...
  // usado para los textfiels con mascaras integrdas
  ...
  static phone(control: AbstractControl): ValidationErrors | null {
    ...
    for (let i = 0; i < control.value.length; i = i + 1) {
      if (all[control.value.substr(0, i)]) {
        const code = control.value.substr(0, i);
        const phone = control.value.substr(code.length, control.value.length);
  ...
  static codeAreaPhone(control: AbstractControl): ValidationErrors | null {
    if (isEmpty(control.value)) {
      return { cbu: true };
    }
    return null;
  ...
  static phoneInvalid(control: AbstractControl): ValidationErrors | null {
    if (isEmpty(control.value)) {
      return { cbu: true };
    }
    return null;
  ...
  static checkboxChecked(control: AbstractControl): ValidationErrors | null {
    if (control.value && typeof control.value === typeof []) {
      const selected = control.value.filter(checkbox => (checkbox.selected || checkbox.indeterminate) && !checkbox.disabled).length;
      if (selected > 0) {
        return null;
      }
...
export class ErrorMessages {
  static messageOf(validatorName: string, validatorValue?: any) {
    const config = {
      ...
      alphanumber: 'Ingresá sólo letras y/o números.',
      ...
      ...(validatorValue && validatorValue.requiredLength) ? {
        ...
        minlength: `Ingresá al menos ${validatorValue.requiredLength} digitos.`,
