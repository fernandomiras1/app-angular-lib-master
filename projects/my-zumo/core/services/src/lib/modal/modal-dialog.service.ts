  ...
  openDialog(componentType: Type<any>, options: ModalDialogOptions) {
    setTimeout(() => {
      ...
      if (options.isOverlay) {
        ...
        this.overlayDiv.id = options.id ? `${options.id}-overlay` : 'overlay';
      ...
      if (options.id) { domElem.id = options.id; }
  ...
  public close(): void {
    setTimeout(() => {
      if (this.dialogComponentRef) {
        if (this.isOverlay && this.documentService.nativeDocument.getElementById(this.overlayDiv.id)) {
